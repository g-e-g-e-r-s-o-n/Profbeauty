...
@section('title')
    პროფბიუთი - სამართავი პანელი | შეთავაზება
        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">
                    <h4 class="card-title">შეთავაზება
                        <!-- Button trigger modal -->
                        <a href="/suggestion" class="btn btn-success float-right" style="font-size: 15px">დამატება</a>
                    </h4>
                    <br>
                    ...
                    <div class="table-responsive">
                        <table class="table">
                            ...
                            @foreach ($suggestions as $row)
                                <tr>
                                    <td>{{ $row->id }}</td>
                                    <td>{{ strlen($row->suggestion_title) > 40 ? substr($row->suggestion_title,0,40).'...' : $row->suggestion_title }}</td>
                                    <td>{{ strlen($row->suggestion_text) > 40 ? substr($row->suggestion_text,0,40).'...' : $row->suggestion_text }}</td>
                                    <td>{{ $row->suggestion_deadline }}</td>
                                    <td>{{ $row->created_at }}</td>
                                    <td>{{ $row->updated_at }}</td>
                                    <td class="text-right">
                                        <a href="/suggestion-edit/{{ $row->id }}" class="btn btn-warning">რედაქტირება</a>
                                    </td>
                                    <td>
                                        <form action="/suggestion-delete/{{ $row->id }}" method="POST">
                                            {{ csrf_field() }}
                                            {{ method_field('DELETE') }}

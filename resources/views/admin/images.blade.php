...
@section('title')
    პროფბიუთი - სამართავი პანელი | ფოტოები
        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">
                    <h4 class="card-title">ფოტოები
                        <!-- Button trigger modal -->
                        <a href="/image" class="btn btn-success float-right" style="font-size: 15px">დამატება</a>
                    </h4>
                    <br>
                    ...
                    <div class="table-responsive">
                        <table class="table">
                            ...
                            @foreach ($images as $row)
                                <tr>
                                    <td>{{ $row->id }}</td>
                                    <td>{{ strlen($row->product->prod_ID) > 20 ? substr($row->product->prod_ID,0,20).'...' : $row->product->prod_ID }}</td>
                                    <td><img src="{{ asset('uploads/image/' . $row->image ) }}" width="120px"></td>
                                    <td>{{ $row->created_at }}</td>
                                    <td>{{ $row->updated_at }}</td>
                                    <td class="text-right">
                                        <a href="/image-edit/{{ $row->id }}" class="btn btn-warning">რედაქტირება</a>
                                    </td>
                                    <td>
                                        <form action="/image-delete/{{ $row->id }}" method="POST">
                                            {{ csrf_field() }}
                                            {{ method_field('DELETE') }}

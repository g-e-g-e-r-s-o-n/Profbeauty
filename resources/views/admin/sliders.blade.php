...
@section('title')
    პროფბიუთი - სამართავი პანელი | სლაიდერი
        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">
                    <h4 class="card-title">სლაიდერი
                        <!-- Button trigger modal -->
                        <a href="/slider" class="btn btn-success float-right" style="font-size: 15px">დამატება</a>
                    </h4>
                    <br>
                    ...
                    <div class="table-responsive">
                        <table class="table">
                            ...
                            @foreach ($sliders as $row)
                                <tr>
                                    <td>{{ $row->id }}</td>
                                    <td>{{ strlen($row->slider_text) > 20 ? substr($row->slider_text,0,20).'...' : $row->slider_text }}</td>
                                    <td>{{ strlen($row->slider_url) > 20 ? substr($row->slider_url,0,20).'...' : $row->slider_url }}</td>
                                    <td><img src="{{ asset('uploads/slider/' . $row->slider_image ) }}" width="120px"></td>
                                    <td>{{ $row->created_at }}</td>
                                    <td>{{ $row->updated_at }}</td>
                                    <td class="text-right">
                                        <a href="/slider-edit/{{ $row->id }}" class="btn btn-warning">რედაქტირება</a>
                                    </td>
                                    <td>
                                        <form action="/slider-delete/{{ $row->id }}" method="POST">
                                            {{ csrf_field() }}
                                            {{ method_field('DELETE') }}
